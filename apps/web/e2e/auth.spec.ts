...
// Auth UI gate (web-v3 P5 / §6.3): redesigned /login and /signup with provider
// buttons, email form, consent notice, states, and the staff pointer to
// /ops/login. The Better Auth backend is Track B; with auth disabled, social/
// magic-link/new-account controls show a "rolling out" state (no dead controls).

test("/login: providers, email form, staff pointer, no 'admin access'", async ({ page }) => {
