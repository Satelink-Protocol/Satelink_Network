// /intelligence/[metric] — one page per catalog metric, generated from the
// live catalog (params seeded from the static fallback so the build is
// offline-safe). Mandatory limitations block for modelled/proxy metrics (§2.5).
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `https://satelink.network/intelligence/${m.slug}` },
  };
}
...
export default async function MetricPage({
  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
      <Link href="/intelligence" className="inline-flex items-center gap-1.5 text-sm text-sl-text-muted hover:text-sl-text">
