...
export function createRpcGateway(db) {
    ...
    router.post('/:chain', creditGate, async (req, res) => {
        ...
        const { chain } = req.params;
        const apiKey = req.headers['x-api-key'];
        // P0 payer-identity (2026-09): x-wallet-address must NEVER reach billing
        // on its own — it was an unauthenticated header trusted to name the
        // deduction target (finding C1). The billing identity is now ONLY a
        // resolved api_key account or a facilitator-verified x402 payment
        // (req.x402.wallet). rawWalletHeader is kept solely for the 401 below
        // and for logging — it is never passed to enforceCapacity/credit_gate.
        ...
        // free-tier logic — never silently downgrade it to anonymous (that
        // would serve the call for free) or bill an unverified wallet.
        ...
        // the per-IP daily limit (over-limit IPs got a 402 there, x402-upgraded
        // by the middleware), so a request reaching this point is under-limit —
        // it flows to the legacy branch below, which rate-limits, meters, and
        // bills $0 (no revenue event).
        //
        ...
        if (canonical && (apiKey || walletForBilling)) {
            ...
            // wallet is walletForBilling ONLY — never the raw header (P0 payer-identity).
            let verdict;
            try {
