// Schema for CONSOLE_ACCOUNTS_V1 — additive only. Never alters api_credits.
// Mirrored in migrations/039_console_accounts_v1.sql; prod has no migration
// runner, so ensureConsoleAccountsSchema() applies it idempotently on first use
// (same boot-DDL pattern as deposit_scan_cursor, migration 038).
//
// Keys are referenced by api_credits.id + a SHA-256 fingerprint. The plaintext
// key is never stored in any of these tables.

export const CONSOLE_ACCOUNTS_DDL = `
...
// The per-request log reads revenue_events_v2 by client_id. Built CONCURRENTLY
// (outside any transaction) so writes to revenue_events_v2 are never blocked.
export const REQUEST_LOG_INDEX_DDL =
  'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_rev2_client_created ON revenue_events_v2 (client_id, created_at DESC)';

let ensured = null;

/** Apply the schema once per process. Safe to call on every request. */
export function ensureConsoleAccountsSchema(pool, { logger = console } = {}) {
  if (!ensured) {
    ensured = (async () => {
      await pool.query(CONSOLE_ACCOUNTS_DDL);
      try {
        await pool.query(REQUEST_LOG_INDEX_DDL);
      } catch (err) {
        // Non-fatal: the log still works (slower) without the index.
        logger.warn?.('[console-accounts] request-log index not built:', err.message);
      }
    })().catch((err) => {
      ensured = null;
      throw err;
...
// Test seam.
export function __resetSchemaCache() { ensured = null; }
