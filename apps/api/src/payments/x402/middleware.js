...
export function createX402Middleware(pool, logger) {
  ...
  return async function x402Middleware(req, res, next) {
    ...
    if (paymentHeader) {
      ...
      // recovered by the CDP facilitator during verify/settle), carried on a
      // trusted request-scoped field. Downstream (rpc_gateway) reads
      // req.x402.wallet — NEVER a client-supplied header — so no request can
      // name a wallet it does not control (P0 payer-identity, 2026-09).
      req.x402 = {
        settled: true,
        txHash: settle.transaction,
        payer: settle.payer,
    ...
    // ---- Rail 1.5 REMOVED (P0 payer-identity, 2026-09) ----
    ...
    // verified x402 payment above (the recovered signer, carried on
    // req.x402.wallet). A request with no valid signed authorization falls
    // through to the 402 challenge below — it is NEVER billed to a
    // header-supplied wallet.
