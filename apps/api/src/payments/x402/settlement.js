  ...
  try {
    ...
    await client.query(
      `INSERT INTO payment_sources (source, amount_usd, token, network, tx_hash, payer, credited_api_key, is_test_data)
       VALUES ('x402', $1, 'USDC', $2, $3, $4, $5, false)`,
      [amountUsd, network, txHash, payer, apiKey]
    );
    ...
    // duplicate guard.
    await client.query(
      `INSERT INTO revenue_events_v2 (op_type, client_id, amount_usdt, status, request_id, created_at, chain, method, source, demand_source)
       VALUES ('rpc_call', $1, $2, 'completed', $3, $4, $5, null, 'x402', 'x402')`,
      [payer, amountUsd, `x402:${txHash}`, Math.floor(Date.now() / 1000), network]
    );
