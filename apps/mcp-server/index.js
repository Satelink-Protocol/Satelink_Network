    ...
    tools: [
      {
        ...
        description:
          "Send a JSON-RPC request to the Satelink Polygon (chain 137) RPC. Free for the first 500 calls/day per network; then pay-per-call. If you receive a 402 Payment Required response, follow the returned instructions to register a free machine key or deposit to continue.",
        inputSchema: {
          type: "object",
          properties: {
            method: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  if (request.params.name === "satelink_rpc_call") {
    ...
    const key = api_key || process.env.SATELINK_API_KEY;
      ...
      if (response.status === 402) {
        return {
          content: [
            {
              type: "text",
              text: `[402 PAYMENT REQUIRED] Your Satelink free tier is exhausted.\n\nTo resume RPC access, either (a) create a free machine key (no wallet, no email), or (b) deposit USDT to the Satelink Revenue Vault on Polygon.\n\nInstructions:\n1. Ensure you are on Polygon (Chain ID 137).\n2. Deposit at least ${responseData.minimum_deposit_usdt || responseData?.deposit?.minimum_usdt || '0.50'} USDT (${responseData.usdt_contract || responseData?.deposit?.usdt_contract}).\n3. Send the deposit to vault: ${responseData.vault_address || responseData?.deposit?.vault_address}\n4. Calldata to use: ${responseData.deposit_url || responseData.calldata_url || responseData?.deposit?.calldata_url || 'N/A'}\n\nFull API Response: ${JSON.stringify(responseData, null, 2)}`
            }
          ],
          isError: true
        };
